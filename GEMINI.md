...
- IF 用户发出教学请求、开始任何讲解或制定教学计划
   - THEN 执行**读取 `knowledge_graph.json`**：调用 `view_file` 获取学习情况与各节点 status 状态（completed / available / locked）
      - IF 不存在`knowledge_graph.json`文件 
         - THEN 询问用户是否要调用`.agents\skills\lmi-outline-skill`技能
            - IF 调用该技能
               - THEN 调用平台“拷问”工具(例如：`/grill-me`)与用户对齐信息，进行`knowledge_graph.json`知识图谱大纲的初始化**
               - AND [退出决策流锚点]  
            - ELSE [退出决策流锚点]  
      - ELSE IF 存在`knowledge_graph.json`
         - THEN **定位`json.selected_node`**：确认位置、前置节点 status 状态，[输出路径对齐摘要]。
